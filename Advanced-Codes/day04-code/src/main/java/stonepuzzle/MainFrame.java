...
public class MainFrame extends JFrame implements KeyListener {
    ...
    int count; //统计步数的变量，成员变量不赋值时，默认为0

    public MainFrame(){

        //窗体对象.addKeyListener(KeyListener实现类对象）
        this.addKeyListener(this);  //this表示当前类对象（窗体对象&KeyListener实现类对象）
    ...
    public void paintView(){
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                JLabel imageLable = new JLabel(new ImageIcon("D:\\image\\"+data[i][j]+".png"));
    ...
    private void move(int keyCode) {
        ...
        if(keyCode==37){
            ...
            //空白块和左侧数据交换
            //data[row][column] data[row+1][column-1];
            int temp = data[row][column];
            data[row][column] = data[row+1][column-1];
            data[row][column-1] = temp;
            column--;
